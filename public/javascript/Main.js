...
const acceptButton = document.getElementById("acceptSearch")
const ul = document.getElementById("ulIngredients");
function timeout () {
...
document.addEventListener("click", function (e) {
  ...
  arrIngredients = JSON.parse(arrIngredients) //arrIngredients from main.ejs file
  ...
  document.addEventListener("input",()=>{
    ...
    for(let i = 0; i<list[0].childNodes.length;++i){ 
        list[0].childNodes[i].addEventListener("click",(mouse)=>{
          let same = false
          if(testarr.length>0){
            
            for(let i = 0; i < testarr.length;++i){
              if(testarr[i] === mouse.target.textContent){
                same = true;
                break;
              }
            }
            ...
            append(mouse.target.textContent)
            testarr.push(mouse.target.textContent)
            ing[0].value =""
...
li.className = "chosenIngredientsList"
